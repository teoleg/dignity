...
renderer, the translation vetting and the model-driven dialogue, with 162
passing tests.
...
- **The Hebrew day starts at sunset.** Date of death is not enough; we need
  ...
  ambiguous. If the hour cannot be found at all, the date given is used
  as it stands and the inscription says so (ADR 0008) — never a silent
  assumption, and the family can swap to the other date in one press.
...
- **An hour nobody can find follows the date as given** (ADR 0008,
  superseding 0007). Blocking meant an old death could never be ordered, and
  asking the family to choose asked them to decide what we had just called
  undecidable. So `derive` settles it: the civil date, no sunset adjustment.
  It is never silent — `DateOfDeath.chosen` carries `by: "family" |
  "as-given"` and the date not used, and the gloss reads "the hour of death
  is unknown and this is the date as given… after sunset it would be X". A
  settled date that renders like a known one is a bug. **The cost is real:
  if the death was after sunset the yahrzeit is a day out.** The page says
  which date is in use and swaps in one press; the library still returns
  `ambiguous` and still refuses to pick — the default lives in `derive`
  alone, beside the words that disclose it.
...
- **What a turn sends is bounded** (`CONTEXT` in `conversation.ts`): the
  recent turns only, each clipped. The transcript is re-sent every turn, so
  an unbounded one makes the price of a turn grow with the conversation and
  charges a pasted wall of text again on every turn after. Trimming is safe
  *because* `describeDraft` carries every established fact above the
  transcript and `derive` decides what is still open — the transcript is for
  tone and for what was just said. Putting facts only in the prose would
  break this.
...
docs/decisions/         architecture decision records
  ...
  0007-an-unknown-hour-is-the-familys-choice.md   (superseded)
  0008-an-unknown-hour-follows-the-date-as-given.md
